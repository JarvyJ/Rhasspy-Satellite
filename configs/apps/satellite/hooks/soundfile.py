...
try:
    # this is an insane monkey patch
...
def write(file, data, samplerate, subtype=None, endian=None, format=None, closefd=True):
    ...
    with SoundFile(file, "w", samplerate, channels, subtype, endian, format, closefd) as f:
        f.write(data)
...
def available_formats():
    ...
    """
    return dict(
        _available_formats_helper(_snd.SFC_GET_FORMAT_MAJOR_COUNT, _snd.SFC_GET_FORMAT_MAJOR)
    )
...
class SoundFile(object):
    ...
    def _check_dtype(self, dtype):
        ...
        try:
            ...
            raise ValueError(
                "dtype must be one of {0!r} and not {1!r}".format(sorted(_ffi_types.keys()), dtype)
            )
